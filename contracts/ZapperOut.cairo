...
@external
func zap_out{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    to_token_address: felt,
    from_pair_address: felt,
    incoming_lp: Uint256,
    ...
    let (tokens_rec: Uint256) = _swap_tokens(
        from_pair_address, amount0, amount1, to_token_address, path0_len, path0, path1_len, path1
    );
...
func _remove_lquidity{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    from_pair_address: felt, incoming_lp: Uint256
    ...
    IERC20.transferFrom(
        contract_address=from_pair_address,
        sender=sender,
    ...
    let (amount0: Uint256, amount1: Uint256) = IRouter.remove_liquidity(
        router,
        token0,
        token1,
        incoming_lp,
...
func _swap_tokens{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    from_pair_address: felt,
    amount0: Uint256,
    amount1: Uint256,
    to_token: felt,
...
func _fill_quote{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    from_token_address: felt, to_token_address: felt, amount: Uint256, path_len: felt, path: felt*
) -> (amount_bought: Uint256) {
    alloc_locals;

    let (contract_address: felt) = get_contract_address();

    let initial_balance: Uint256 = IERC20.balanceOf(
        contract_address=to_token_address, account=contract_address
    );
