...
func constructor{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    router: felt, initial_owner: felt
) {
...
// @return router
@view
func router{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}() -> (router: felt) {
...
// @return goodwill
@view
func goodwill{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}() -> (
    goodwill: felt
) {
...
@external
func update_goodwill{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    new_goodwill: felt
) {
    alloc_locals;
    _only_owner();
...
@external
func zap_in{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    from_token_address: felt,
    pair_address: felt,
    amount: Uint256,
    min_pool_token: Uint256,
    path_len: felt,
    path: felt*,
    transfer_residual: felt,
) -> (lp_bought: Uint256) {
    alloc_locals;
    ...
    let (lp_bought: Uint256) = _perform_zap_in(
        from_token_address, pair_address, amount_to_invest, path_len, path, transfer_residual
    );
...
func _perform_zap_in{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    from_token_address: felt,
    pair_address: felt,
    amount: Uint256,
    path_len: felt,
    ...
    if (from_token_address == token0) {
        ...
        if (from_token_address == token1) {
            ...
            let (temp_amt: Uint256, temp_token: felt) = _fill_quote(
                from_token_address, pair_address, amount, path_len, path
            );
    ...
    let (token0_bought: Uint256, token1_bought: Uint256) = _swap_intermediate(
...
func _jedi_deposit{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    token0: felt,
    token1: felt,
    token0_bought: Uint256,
    token1_bought: Uint256,
    transfer_residual: felt,
) -> (liquidity: Uint256) {
    alloc_locals;
    ...
    let (amountA: Uint256, amountB: Uint256, liquidity: Uint256) = IRouter.add_liquidity(
        ...
        tokenA=token0,
        tokenB=token1,
        amountADesired=token0_bought,
        amountBDesired=token1_bought,
...
func _get_pair_tokens{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    pair_address: felt
...
func _fill_quote{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    from_token_address: felt, pair_address: felt, amount: Uint256, path_len: felt, path: felt*
) -> (amount_bought: Uint256, intermediate_token: felt) {
    alloc_locals;

    let (token0, token1) = _get_pair_tokens(pair_address);
    let (contract_address) = get_contract_address();

    let initial_balance0: Uint256 = IERC20.balanceOf(
        contract_address=token0, account=contract_address
    );
    let initial_balance1: Uint256 = IERC20.balanceOf(
        contract_address=token1, account=contract_address
    );
    ...
    let (amounts_len: felt, amounts) = IRouter.swap_exact_tokens_for_tokens(
        contract_address=router,
        amountIn=amount,
    ...
    let contract_token0_balance: Uint256 = IERC20.balanceOf(
        contract_address=token0, account=contract_address
    );
    let final_balance0: Uint256 = uint256_checked_sub_le(contract_token0_balance, initial_balance0);

    let contract_token1_balance: Uint256 = IERC20.balanceOf(
        contract_address=token1, account=contract_address
    );
    ...
    local intermediate_token: felt;
    let (is_final_balance1_less_than_equal_final_balance0) = uint256_le(
        final_balance1, final_balance0
    );
...
func _swap_intermediate{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    intermediate_token: felt, token0: felt, token1: felt, amount: Uint256
) -> (token0_bought: Uint256, token1_bought: Uint256) {
    alloc_locals;
    let factory: felt = _jedi_factory.read();
    let pair_address: felt = IFactory.get_pair(
...
func _token_to_token{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    from_token: felt, to_token: felt, token_to_trade: Uint256
) -> (token_bought: Uint256) {
    alloc_locals;
    ...
    let pair_address: felt = IFactory.get_pair(
        contract_address=factory, token0=from_token, token1=to_token
    );
    assert_not_zero(pair_address);
...
func _calculate_swap_in_amount{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    reserve_in: Uint256, user_in: Uint256
) -> (amount_to_swap: Uint256) {
    alloc_locals;
    ...
    let user_in_mul_3988000_add_reserve_in_mul_3988009: Uint256 = uint256_checked_add(
        user_in_mul_3988000, reserve_in_mul_3988009
    );
    let reserve_in_mul_user_in_mul_3988000_add_reserve_in_mul_3988009: Uint256 = uint256_checked_mul(
        reserve_in, user_in_mul_3988000_add_reserve_in_mul_3988009
    );
    ...
    let (amount_to_swap: Uint256, _) = uint256_unsigned_div_rem(
        sqrt_sub_reserve_in_mul_1997, Uint256(1994, 0)
    );
